...
func (s LeaseStore) Acquire(
	...
	p := makeInternalPlanner("lease-acquire", txn, security.RootUser, nil, s.memMetrics)
	...
	err = s.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		p := makeInternalPlanner("lease-insert", txn, security.RootUser, nil, s.memMetrics)
		defer finishInternalPlanner(p)
...
func (s LeaseStore) Release(ctx context.Context, stopper *stop.Stopper, lease *LeaseState) {
	...
	for r := retry.Start(retryOptions); r.Next(); {
		...
		err := s.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
			...
			p := makeInternalPlanner("lease-release", txn, security.RootUser, nil, s.memMetrics)
			defer finishInternalPlanner(p)
...
func (s LeaseStore) countLeases(
	...
	var count int
	err := s.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		p := makeInternalPlanner("leases-count", txn, security.RootUser, nil, s.memMetrics)
		defer finishInternalPlanner(p)
